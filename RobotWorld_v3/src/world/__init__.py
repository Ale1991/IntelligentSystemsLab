...
class World(threading.Thread):
    ...
    def listen_body_request(self):
        for action_str in self._bodyPubSub.listen(): # start listening
            if action_str["type"] == "message":
                ...
                if "RobotBodyInfo" in jData: # se trova una RobotBodyInfo
                    if jData["RobotBodyInfo"] == "RobotState":
    ...
    def updateSensor(self):
        if self.robotDirection != None and self.robotX != None and self.robotY != None:
            sensor_array = self.calc_robot_sonar(self.robotX, self.robotY, self.robotDirection)
            jsn = json.dumps({ 'worldResponse': 'updateSensorResponse', 'x': self.robotX, 'y': self.robotY, 'sensor_array': sensor_array})
            self._Redis.publish(CH_BODY_WORLD, jsn)
